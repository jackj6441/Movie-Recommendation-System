# PRD: Conversational RAG — Context Resolution & Multi-Turn UX (v1.1)
...
3. Whole trimmed `message` as single catalog search query (case-insensitive substring rules unchanged). If **≥1** hit, use the **first** hit as seed (no multi-hit disambiguation). If **0** hits, continue to bootstrap/clarify/disambiguate paths below.
...
| Whole-message search multi-hit | **A** — if search returns ≥1 hit, take **first** catalog hit and proceed Ready; **only 0 hits** lead to `no_resolvable_seeds` disambiguation (up to 10 candidates) |
...
- **Taste rail** — seeds, genres, optional single **Year** chip when `year_min` / `year_max` set; **× removes immediately** per v1.1 rules plus **last-chip** behavior (v1.2 table). User-turn lines unchanged.
...
- Resolve reasons: `explicit_seed`, `quoted_title`, `whole_message_search`, `genre_bootstrap`, `no_resolvable_seeds`, `missing_genre_and_title`, `invalid_genre`, `empty_recommendations`
...
| **Frontend** | canSend rules; user turn strings; picker limit 5; genre-only send; taste rail sticky (layout); poster disambiguation; scroll after more-like; remove last seed/genre | `App.test.tsx`, chat mocks |
